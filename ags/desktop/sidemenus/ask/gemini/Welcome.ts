...
export const ConfigToggle = ({
  ...
  }, toggleIcon).hook(enabled, (self: BoxProps) =>
  ...
  }, toggleButtonIndicator).hook(enabled, (self: BoxProps) =>
  ...
  const chatSettings = VBox(
    ...
    VBox(
      ...
      ConfigToggle({
        ...
        onChange(_, newValue: boolean) { GeminiService.assistantPrompt = newValue },
        desc: "Tells Gemini:\n- It's a Linux sidebar assistant\n- Be brief and use bullet points",
      }),
      ConfigToggle({
        name: 'Safety',
        initValue: GeminiService.safe,
        onChange(_, newValue: boolean) { GeminiService.safe = newValue },
        desc: 'When turned off, tells the API (not the model) \nto not block harmful/explicit content',
      }),
      ConfigToggle({
        name: 'History',
        initValue: GeminiService.useHistory,
        onChange(_, newValue: boolean) { GeminiService.useHistory = newValue },
