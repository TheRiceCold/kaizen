import { 
  ls, expandTilde, 
  couldBeMath, execAndClose, 
  launchCustomCommand, hasUnterminatedBackslash,
} from './miscfunctions.js'
import {
  ExecuteCommandButton, SearchButton,
  DirectoryButton, DesktopEntryButton, 
  CalculationResultButton, CustomCommandButton,
} from './searchbuttons.js';
import { checkKeybind } from './Keybind'
...
export const SearchAndWindows = () => {
  ...
  const entry = Widget.Entry({
    ...
    onChange: (entry) => { // this is when you type
      ...
      _appSearchResults = apps.query(text)
      ...
      if (isDir) {
        ...
        contents.forEach((item) => resultsBox.add(DirectoryButton(item)))
      }
  ...
  return Widget.Box({
    ...
    children: [
      ClickToClose({ // Top margin. Also works as a click-outside-to-close thing
        child: Widget.Box({ className: 'bar-height' })
      }),
      Widget.Box({
        hpack: 'center',
        children: [
          ...
          Widget.Box({
            ...
            setup: self => self.pack_start(entryPromptRevealer, true, true, 0),
          }),
          entryIcon,
        ]
      }),
